import {analyzeOutcomes,predictOutcome} from './outcome-engine.mjs';
...
export function playerProfile(rows,player,position='ANY'){
 const all=rows.filter(r=>r.player===player),pos=position==='ANY'?all:all.flatMap(r=>{const part=r.roleStats?.find(s=>s.position===position);return part?[{...r,position,rating:part.stats.rating?.value,attack:part.stats.shots?.xg,defense:(part.stats.interceptions?.won||0)+(part.stats.blocks?.total||0)+(part.stats.saves?.total||0)}]:r.position===position?[r]:[];}),chosen=pos.length?pos:all,base=mean(rows.map(r=>r.rating))??6;
...
export function balancedTeams(rows,selected,{gk=1,df=2,mf=2,fw=2,ratingWeight=1,attackWeight=0,defenseWeight=0,chemistryWeight=0,attempts=2500,outcomeMode='rating'}={}){
 ...
 const analysis=outcomeMode==='rating'?null:analyzeOutcomes(rows),useOutcome=!!analysis&&analysis.model.games>=3&&(outcomeMode==='experimental'||analysis.validation.available&&analysis.validation.better);
 ...
 for(const indexes of combinations.slice(0,attempts)){tried++;const ix=new Set(indexes),w=fit([...fixedWest,...free.filter((_,i)=>ix.has(i))]),e=fit([...fixedEast,...free.filter((_,i)=>!ix.has(i))]);if(!w||!e)continue;
 // Improve ANY assignments by legal within-team position swaps for the outcome objective.
 if(useOutcome){for(let round=0;round<2;round++){let improved=false;for(const team of [w,e])for(let a=0;a<team.players.length;a++)for(let b=a+1;b<team.players.length;b++){const pa=team.players[a],pb=team.players[b];if(pa.position===pb.position||selected.find(p=>p.player===pa.player).position!=='ANY'||selected.find(p=>p.player===pb.player).position!=='ANY')continue;const before=Math.abs(predictOutcome(analysis.model,w.players,e.players).score);const na=profiles.get(pa.player)[pb.position],nb=profiles.get(pb.player)[pa.position];if(Number(na.fallback)+Number(nb.fallback)>Number(pa.fallback)+Number(pb.fallback))continue;team.players[a]=na;team.players[b]=nb;const after=Math.abs(predictOutcome(analysis.model,w.players,e.players).score);if(after+1e-6<before)improved=true;else{team.players[a]=pa;team.players[b]=pb;}}if(!improved)break;}}
 const prediction=analysis?predictOutcome(analysis.model,w.players,e.players):null;
 const westScore=score(w.players),eastScore=score(e.players),wc=chemistryWeight?chemistry(w.players):0,ec=chemistryWeight?chemistry(e.players):0,objective=useOutcome?(prediction.coverage<.25?2:0)+Math.abs(prediction.score)+.01*[...w.players,...e.players].filter(p=>p.fallback).length:Math.abs(westScore-eastScore)+.2*(w.cost+e.cost)-chemistryWeight*(wc+ec);const key=[w.players.map(p=>p.player).sort().join(','),e.players.map(p=>p.player).sort().join(',')].sort().join('|');if(seen.has(key))continue;seen.add(key);solutions.push({west:w.players,east:e.players,westScore,eastScore,gap:Math.abs(westScore-eastScore),objective,chemistry:wc+ec,prediction});}
 solutions.sort((a,b)=>a.objective-b.objective);if(!solutions.length)throw new Error('固定チーム・ポジション条件を満たす候補がありません。条件を緩めてください');return{solutions:solutions.slice(0,5),tried,total:combinations.length,useOutcome,validation:analysis?.validation,outcomeGames:analysis?.model.games};
}
